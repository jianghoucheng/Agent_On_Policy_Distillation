...
We provide an on-policy distillation recipe that injects step-level reverse-KL rewards from a stronger teacher model. The student still performs multi-turn tool calling, but each tool action receives its own advantage signal (text + tool arguments) thanks to the contiguous-response segmentation.

```bash
bash recipe/retool/run_qwen2-32b_opd.sh
```

Key requirements:

- Set `TEACHER_HF_ID` in the script to the HuggingFace ID of your teacher checkpoint. The current implementation assumes the teacher shares the same tokenizer with the student (e.g., different sizes of the same family).
- Enable rollout log-prob collection via `actor_rollout_ref.rollout.calculate_log_probs=True` (already part of the script) so the reward manager can recover the student token probabilities used in reverse KL.
- Use the new reward manager `reward_model.reward_manager=on_policy_distill` together with `algorithm.adv_estimator=token_rkl`, which directly feeds the token-level reverse-KL rewards into PPO.
- To avoid duplicated loading of a large teacher and to keep it on the intended GPUs, you can leave the defaults (`teacher_init_on_call=false`, `reuse_teacher=true`). Optional `teacher_device_map='auto'` lets `AutoModelForCausalLM` shard across visible GPUs if you cannot dedicate an extra card to the teacher.
- Adjust `reward_model.reward_kwargs` if you need different teacher devices、batch sizes、token权重（例如 `tool_reward_weight`、`final_answer_weight`）等。
