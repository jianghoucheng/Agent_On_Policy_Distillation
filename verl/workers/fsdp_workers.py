...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if config.model.get("as_teacher_lm", False):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            if self.config.model.get("as_teacher_lm", False):
                # Teacher LM: return per-token log-probs
                ...
                teacher_log_probs = torch.cat([pad, gathered], dim=1)  # (bsz, seqlen)
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if self.config.model.get("as_teacher_lm", False):
                # scores: (bsz, seq_len) token log-probs
                output = DataProto.from_dict(tensors={"teacher_log_probs": scores})
            else:
