...
@register_adv_est(AdvantageEstimator.TOKEN_RKL)
def compute_token_rkl_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    **kwargs,
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Use token-level rewards directly as advantages/returns for reverse-KL distillation.

    Args:
        token_level_rewards: Raw rewards per token (e.g., student_log_prob - teacher_log_prob).
        response_mask: Binary mask that identifies valid response tokens.

    Returns:
        advantages: token_level_rewards masked to valid tokens.
        returns: identical to advantages (outcome = per-token).
    """
