...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    if "ref_log_prob" not in batch.batch.keys():
        raise KeyError(
            '"ref_log_prob" missing in batch.batch. OPD/token_kl_reg requires teacher log probs; '
            "ensure the teacher model is initialized and ref log probs are computed."
        )
    if "old_log_probs" not in batch.batch.keys():
        raise KeyError('"old_log_probs" missing in batch.batch, cannot compute token-level KL metrics.')

    teacher_log_prob = batch.batch["ref_log_prob"]
    student_log_prob = batch.batch["old_log_probs"]
    token_kl = (teacher_log_prob - student_log_prob) * response_mask
    valid_token_kl = torch.masked_select(token_kl, response_mask)
    if valid_token_kl.numel() > 0:
        metrics.update(
                {
                    "actor/token_kl/mean": torch.mean(valid_token_kl).detach().item(),
                    "actor/token_kl/max": torch.max(valid_token_kl).detach().item(),
                    "actor/token_kl/min": torch.min(valid_token_kl).detach().item(),
                    "actor/token_kl/std": torch.std(valid_token_kl).detach().item(),
                }
            )
